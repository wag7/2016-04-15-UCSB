...
gap2007 = subset(gapminder, subset = year == 2007) 
str(gap2007)
```
...
Say, for example, that we want R to print a message if the variable we just created has a has a particular value. The value I created was called `gap2007`
...
We'll make a n case you wanted to just do a quick check to see if 2002 was even in the gapminder data, you could use `any` or `%in%`
